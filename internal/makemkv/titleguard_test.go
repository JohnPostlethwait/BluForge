...
// The snapshot is the enumeration a kill is logged against: every title the
// guard saw, index and playlist, in index order.
func TestGuardSnapshotListsTheEnumerationInOrder(t *testing.T) {
	g := newTitleGuard(3, "00001.mpls")
	g.observe(added("00001.mpls", 3))
	g.observe(added("00102.mpls", 0))
	g.observe(added("00871.mpls", 1))

	snap := g.snapshot()
	if len(snap) != 3 {
		t.Fatalf("snapshot has %d titles, want 3: %+v", len(snap), snap)
	}
	if snap[0].Index != 0 || snap[0].Source != "00102.mpls" {
		t.Errorf("first entry = %+v, want index 0, 00102.mpls (sorted by index)", snap[0])
	}
	if snap[2].Index != 3 || snap[2].Source != "00001.mpls" {
		t.Errorf("last entry = %+v, want index 3, 00001.mpls", snap[2])
	}
}
